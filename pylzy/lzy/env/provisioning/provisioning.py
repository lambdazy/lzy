...
_Req = Union[AnyRequirement, NotSpecifiedType]
...
class Provisioning(BaseProvisioning):
    ...
    def _as_vm_spec(self) -> VmSpec:
        return VmSpec(
            cpu_type=_coerce(self.cpu_type, ''),
            gpu_type=_coerce(self.gpu_type, ''),
    ...
    def _filter_spec(self, prov: VmSpec, spec: VmSpec) -> bool:
        ...
        if (
            prov.cpu_type and prov.cpu_type != spec.cpu_type or
            prov.gpu_type and prov.gpu_type != spec.gpu_type
        ):
            return False
