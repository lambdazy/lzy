...
class ModulesSearchTests(TestCase):
    def test_modules_search(self):
        ...
        # Act
        remote, local = select_modules({"level1": level1})
        ...
        self.assertFalse(
            directory + "/test_modules/level1/level2/level3/level3.py" in local
        )
    ...
    def test_modules_search_2(self):
        _, local = select_modules({"level_foo": level_foo})
        os.chdir(os.path.dirname(__file__))
        directory = os.path.abspath(os.path.join(os.getcwd(), os.pardir))
        self.assertTrue(directory + "/test_modules" in local)
        self.assertFalse(directory + "/test_modules/level1" in local)
    ...
    def test_modules_search_3(self):
        _, local = select_modules({"bar": bar})
        os.chdir(os.path.dirname(__file__))
        cwd = os.getcwd()
