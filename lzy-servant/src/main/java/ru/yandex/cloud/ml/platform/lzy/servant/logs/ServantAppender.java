...
public class ServantAppender extends AbstractAppender {
    ...
    private void log(String eventId, String eventDate, String level, String logger, String message) throws SQLException {
        Connection connection = ConnectionFactory.getDatabaseConnection();
        PreparedStatement stmt = connection.prepareStatement(
                "INSERT INTO lzy.servant_logs (EVENT_ID, EVENT_DATE, LEVEL, LOGGER, MESSAGE, SERVANT_ID) VALUES (?, ?, ?, ?, ?, ?)"
        );
    ...
    private void queueThread(){
        while (true){
            ...
            try {
                log(eventId.toString(), new Date(event.getTimeMillis()).toString(), event.getLevel().toString(), event.getLoggerFqcn(), event.getMessage().getFormattedMessage());
