...
public abstract class BaseTransmitterFactory {
    ...
    public UploadTransmitter sameThreadUploadTransmitter() {
        return create(createByteBufferPool("upload", byteBufferSizeType, 1), newDirectExecutorService(), newDirectExecutorService(),
                newDirectExecutorService());
    }

    public UploadTransmitter fixedPoolsUploadTransmitter(String transmitterName, int uploadsPoolSize, int chunksPoolSize) {
        ...
        return create(createByteBufferPool(transmitterName, byteBufferSizeType, uploadsPoolSize + chunksPoolSize),
                fixedThreadPool(transmitterName, uploadsPoolSize, "upload", poolNumber), fixedThreadPool(transmitterName, chunksPoolSize, "chunks", poolNumber),
                newDirectExecutorService());
    }
    ...
    public DownloadTransmitter fixedPoolsDownloadTransmitter(String transmitterName, int downloadsPoolSize, int chunksPoolSize) {
        ...
        return create(createByteBufferPool(transmitterName, byteBufferSizeType, downloadsPoolSize + chunksPoolSize),
                fixedThreadPool(transmitterName, downloadsPoolSize, "download", poolNumber), fixedThreadPool(transmitterName, chunksPoolSize, "chunks", poolNumber),
    ...
    public Transmitter fixedPoolsTransmitter(String transmitterName, int downloadsPoolSize, int chunksPoolSize) {
        final long poolNumber = POOL_NUMBER.incrementAndGet();
        return create(createByteBufferPool(transmitterName, byteBufferSizeType, downloadsPoolSize + chunksPoolSize),
                fixedThreadPool(transmitterName, downloadsPoolSize, "transmitter", poolNumber), fixedThreadPool(transmitterName, chunksPoolSize, "chunks", poolNumber),
                fixedThreadPool(transmitterName, downloadsPoolSize, "consumer", poolNumber));
    }

    protected ListeningExecutorService fixedThreadPool(String transmitterName, int size, String poolName, long poolNumber) {
    ...
    protected ListeningExecutorService createSingleThreadPool(String poolName, long poolNumber) {
        final BasicThreadFactory threadFactory = new BasicThreadFactory.Builder()
                ...
                .priority(Thread.NORM_PRIORITY)
                .build();
        return listeningDecorator(new ThreadPoolExecutor(1, 1, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<>(), threadFactory));
    }

    protected ThreadPoolExecutor createFixedThreadPool(String transmitterName, int size, String poolName, long poolNumber, ThreadFactory threadFactory) {
        return new ThreadPoolExecutor(size, size, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<>(), threadFactory);
    }

    protected ByteBufferPool createByteBufferPool(String transmitterName, ByteBufferSizeType byteBufferSizeType, int size) {
