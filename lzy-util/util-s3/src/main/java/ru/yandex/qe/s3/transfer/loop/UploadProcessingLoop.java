...
public abstract class UploadProcessingLoop extends ProcessingLoop<UploadState> {
    ...
    protected abstract void uploadObject(String bucket, String key, Metadata metadata, byte[] buffer, int offset, int length, ConcurrencyConflictResolve concurrencyConflictResolve, @Nullable DateTime expirationTime);

    protected abstract void initMultiPartUpload(String bucket, String key, Metadata metadata, ConcurrencyConflictResolve concurrencyConflictResolve, @Nullable DateTime expirationTime);
                        ...
                        if (nextPartNumber == 1) {
                            if (byteBuffer.limit() < byteBuffer.capacity()) {
                                if (filled <= 0 && !request.isAllowEmptyStream()) {
                                    throw new IllegalArgumentException(format("input for %s:%s is empty!", request.getBucket(), request.getKey()));
                                }
        ...
        return new FutureCallback<Void>() {
            ...
            public void onFailure(Throwable throwable) {
                if (throwable instanceof CancellationException) {
                    LOG.info("canceled upload part {} for {}:{}", chunkRunnable.getPartNumber(), request.getBucket(), request.getKey());
                } else {
                    LOG.warn("fail to upload part {} for {}:{}, implementation specific info: {}", chunkRunnable.getPartNumber(), request.getBucket(), request.getKey()
    ...
    private InputStream boundIfLengthSpecifiedAndLogStatOnClose() {
        ...
        final long contentLength = request.getObjectMetadata().getObjectContentLength();
        return contentLength > 0 ? new LengthCheckInputStream(new BoundedInputStream(inputStream, contentLength), contentLength, false) : inputStream;
    }
    ...
    private void putObject(ByteBuffer byteBuffer, Stopwatch stopwatch) {
        ...
        uploadObject(request.getBucket(), request.getKey(), objectMetadata, byteBuffer.array(), 0, byteBuffer.limit(), request.getConcurrencyConflictResolve(),
    ...
    private void initUpload() {
        initMultiPartUpload(request.getBucket(), request.getKey(), request.getObjectMetadata(), request.getConcurrencyConflictResolve(),
                (request instanceof TTLUploadRequest) ? DateTime.now().plus(((TTLUploadRequest) request).getTTL()) : null);
    ...
    private void handleInterrupt(Stopwatch stopwatch) {
        ...
        LOG.warn("interrupted upload for {}:{}; upload took {} ms, cancellation took {} ms", request.getBucket(), request.getKey(),
    ...
    private void handleThrowable(Throwable throwable, Stopwatch stopwatch) {
        ...
        LOG.error("fail upload for {}:{}: upload took {} ms, handling failure took {} ms", request.getBucket(), request.getKey(),
    ...
    private void abortUpload(TransferStatus finalStatus, Stopwatch stopwatch) {
        ...
        if (Thread.interrupted()) {
            LOG.warn("aborting upload of {}:{}: cleared transmitter thread interrupt flag", request.getBucket(), request.getKey());
        }

        try {
            abortUpload(request.getBucket(), request.getKey());
        } catch (Exception e) {
            LOG.warn("caught exception while aborting upload for {}:{}; ignored", request.getBucket(), request.getKey(), e);
        }
    ...
    private class UploadChunkTask extends ChunkRunnable {
        ...
        protected void doRun() {
            ...
            checkCanceled();
            LOG.debug("start upload part {} size {} for {}:{}", partNumber, byteBuffer.limit(), request.getBucket(), request.getKey());
