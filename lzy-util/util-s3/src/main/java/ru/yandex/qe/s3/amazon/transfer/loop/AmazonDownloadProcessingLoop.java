...
public class AmazonDownloadProcessingLoop<T> extends DownloadProcessingLoop<T> {
    ...
    public AmazonDownloadProcessingLoop(AmazonS3 amazonS3, RetryPolicy streamReadFailRetryPolicy, ByteBufferPool byteBufferPool,
                                        ListeningExecutorService taskExecutor, ListeningExecutorService consumeExecutor, DownloadRequest request, Function<MetaAndStream, T> processor,
    ...
    @Override
    protected void consumeContent(String bucket, String key, long rangeStart, long rangeEnd, int partNumber, ThrowingConsumer<InputStream> consumer) {
        ...
        while (true) {
            if (exception != null) {
                Uninterruptibles.sleepUninterruptibly(delayBeforeNextRetry(request, exception, attempt), TimeUnit.MILLISECONDS);
            }
            try (InputStream chunkStream = amazonS3.getObject(request).getObjectContent()) {
                ...
                if (!shouldRetry(request, exception, attempt)) {
                    throw new S3TransferException(format("%s during download part %s size %s bytes for %s:%s, was %s retries",
    ...
    private boolean shouldRetry(GetObjectRequest request, Exception exception, int retriesAttempted) {
        return retriesAttempted <= streamReadFailRetryPolicy.getMaxErrorRetry() &&
                streamReadFailRetryPolicy.getRetryCondition().shouldRetry(request, new AmazonClientException(exception), retriesAttempted);
    }

    private long delayBeforeNextRetry(GetObjectRequest request, Exception exception, int retriesAttempted) {
        return streamReadFailRetryPolicy.getBackoffStrategy().delayBeforeNextRetry(request, new AmazonClientException(exception), retriesAttempted);
    }
