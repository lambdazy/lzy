...
public class AmazonTTLTransmitterFactory extends AmazonTransmitterFactory {
    ...
    public AmazonTTLTransmitterFactory(@Nonnull AmazonS3 amazonS3, String s3Endpoint, @Nonnull TTLRegister ttlRegister) {
        super(ByteBufferSizeType._8_MB, amazonS3);
        this.s3Endpoint = s3Endpoint;
        this.ttlRegister = ttlRegister;
    }

    public AmazonTTLTransmitterFactory(@Nonnull  ByteBufferSizeType bufferSizeType, @Nonnull AmazonS3 amazonS3, String s3Endpoint, @Nonnull TTLRegister ttlRegister) {
        super(bufferSizeType, amazonS3);
        this.s3Endpoint = s3Endpoint;
        this.ttlRegister = ttlRegister;
    }

    public TTLTransmitter create(@Nonnull Duration defaultTTL, @Nonnull RetryPolicy retryPolicy, @Nonnull ByteBufferPool byteBufferPool, @Nonnull ListeningExecutorService transferExecutor,
                                 @Nonnull ListeningExecutorService chunksExecutor, @Nonnull ListeningExecutorService consumeExecutor) {
        return new BaseTTLTransmitter(ttlRegister, defaultTTL, byteBufferPool, transferExecutor, chunksExecutor, consumeExecutor) {
            @Nonnull
            @Override
            ...
            @Override
            public UploadProcessingLoop create(@Nonnull UploadRequest uploadRequest, @Nullable Consumer<UploadState> progressListener, @Nullable Executor executor, @Nonnull ByteBufferPool byteBufferPool, @Nonnull ListeningExecutorService chunksExecutor) {
                return new AmazonUploadProcessingLoop(amazonS3, byteBufferPool, chunksExecutor, uploadRequest, progressListener, executor);
            }
        };
    }

    public TTLTransmitter sameThreadTTLTransmitter(@Nonnull Duration defaultTTL) {
        return create(defaultTTL, new RetryPolicy(DEFAULT_RETRY_CONDITION, DEFAULT_BACKOFF_STRATEGY, PredefinedRetryPolicies.DEFAULT_MAX_ERROR_RETRY, false),
                createByteBufferPool("ttl-transmitter", byteBufferSizeType, 1), newDirectExecutorService(), newDirectExecutorService(),
                newDirectExecutorService());
    }

    public TTLTransmitter fixedPoolsTTLTransmitter(@Nonnull String transmitterName, @Nonnull Duration defaultTTL, int downloadsPoolSize, int chunksPoolSize) {
        final long poolNumber = POOL_NUMBER.incrementAndGet();
        return create(defaultTTL,
                new RetryPolicy(DEFAULT_RETRY_CONDITION, DEFAULT_BACKOFF_STRATEGY, PredefinedRetryPolicies.DEFAULT_MAX_ERROR_RETRY, false),
    ...
    public TTLTransmitter sameThreadTTLTransmitter(@Nonnull Duration defaultTTL, RetryPolicy streamReadFailRetryPolicy) {
        return create(defaultTTL, streamReadFailRetryPolicy, createByteBufferPool("ttl-transmitter", byteBufferSizeType, 1),
                newDirectExecutorService(), listeningDecorator(Executors.newSingleThreadExecutor()), listeningDecorator(Executors.newSingleThreadExecutor()));
    }

    public TTLTransmitter sameThreadTTLTransmitter(@Nonnull Duration defaultTTL, int streamReadFailRetryCount) {
        return sameThreadTTLTransmitter(defaultTTL, new RetryPolicy(DEFAULT_RETRY_CONDITION, DEFAULT_BACKOFF_STRATEGY, streamReadFailRetryCount, false));
    }

    public TTLTransmitter fixedPoolsTTLTransmitter(@Nonnull String transmitterName, @Nonnull Duration defaultTTL, int downloadsPoolSize, int chunksPoolSize, int streamReadFailRetryCount) {
    ...
    public TTLTransmitter fixedPoolsTTLTransmitter(@Nonnull String transmitterName, @Nonnull Duration defaultTTL, int downloadsPoolSize, int chunksPoolSize, RetryPolicy streamReadFailRetryPolicy) {
        final long poolNumber = POOL_NUMBER.incrementAndGet();
