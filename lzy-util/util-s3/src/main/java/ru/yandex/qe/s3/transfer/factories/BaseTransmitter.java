...
public abstract class BaseTransmitter implements Transmitter {
    ...
    public abstract <T> DownloadProcessingLoop<T> create(@Nonnull DownloadRequest downloadRequest, @Nonnull ThrowingFunction<MetaAndStream, T> processor,
    ...
    public ListenableFuture<UploadState> upload(@Nonnull UploadRequest request,
                                                @Nullable Consumer<UploadState> progressListener, @Nullable Executor notifyExecutor) {
        return transferExecutor.submit(create(request, progressListener, notifyExecutor, byteBufferPool, chunksExecutor));
    }


    @Override
    public <T> ListenableFuture<DownloadResult<T>> downloadF(@Nonnull DownloadRequest request, @Nonnull ThrowingFunction<MetaAndStream, T> processor) {
        //noinspection ConstantConditions
    ...
    public <T> ListenableFuture<DownloadResult<T>> downloadF(@Nonnull DownloadRequest request, @Nonnull ThrowingFunction<MetaAndStream, T> processor,
                                                             @Nullable Consumer<DownloadState> progressListener, @Nullable Executor notifyExecutor) {
        return transferExecutor.submit(create(request, processor, progressListener, notifyExecutor, byteBufferPool, chunksExecutor, consumeExecutor));
    }

    @Override
    public ListenableFuture<DownloadResult<Void>> downloadC(@Nonnull DownloadRequest request, @Nonnull ThrowingConsumer<MetaAndStream> consumer,
